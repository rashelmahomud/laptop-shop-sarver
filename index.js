...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.paezf.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const db = client.db("laptop-server");
    ...
    const blogsReviewCollection = db.collection("blogreview");
    ...
    app.put("/service/:id", async (req, res) => {
      ...
      const updatedDoc = { $set: { Quantity } };
      const result = await serviceCollection.updateOne(filter, updatedDoc, { upsert: true });
      res.send(result);
    });
    ...
    app.post("/blogreview", async (req, res) => {
      const blog = req.body;
      const result = await blogsReviewCollection.insertOne(blog);
      res.send(result);
    });
    ...
    // Keep connection open (don’t close client here for long-running server)
  }
}
